...
double vec_hypot(double x, double y);
...
void Vector_Normal(Vector *result, Vector *vec) {
  double mag =  vec_hypot(vec->x, vec->y);
  result->x = vec->x / mag;
  result->y = vec->y / mag;
}

void Vector_SetMag(Vector *result, Vector *vec, double mag) {
  double pMag =  vec_hypot(vec->x, vec->y);
...
double Vector_Dist(Vector *a, Vector *b) {
  return  vec_hypot(a->x - b->x, a->y - b->y);
}
...
void Vector_Truncate(Vector *result, Vector *vec, double limit) {
  ...
  if (result->x*result->x + result->y*result->y > limit * limit) {
    double factor = limit/ vec_hypot(vec->x, vec->y);
    result->x *= factor;
    result->y *= factor;
  }
}
...
double Vector_ProjectionV(Vector *vec, Vector *line) {
  return (vec->x*line->x+vec->y*line->y) /  vec_hypot(line->x, line->y);
}

double Vector_ProjectionA(Vector *vec, double angle) {
  return  vec_hypot(vec->x, vec->y) *  cos(angle);
}

double Vector_PerpProjection(Vector *vec, Vector *line) {
  return  fabs(vec->x * line->y - vec->y * line->x) /  vec_hypot(line->x, line->y);
}

void Vector_ComponentV(Vector *result, Vector *vec, Vector *line) {
  double mag = (vec->x*line->x+vec->y*line->y) /  vec_hypot(line->x, line->y);
  result->x = mag * line->x;
  result->y = mag * line->y;
}

void Vector_ComponentA(Vector *result, Vector *vec, double angle) {
  double r =  vec_hypot(vec->x, vec->y) *  cos(angle);
...
double Vector_Compare(Vector *a, Vector *b) {
  return  vec_hypot(a->x, a->y) -  vec_hypot(b->x, b->y);
}
